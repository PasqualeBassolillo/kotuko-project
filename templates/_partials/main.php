<main id="main">
    <section class="header-section section-site-a">
        <div class="container">
            <div class="row">
                <div class="col-12 title-wrapper">
                    <h1 class="section-title">Kaleidon, risveglia il tuo<br><span>Equilibrio</span></h1>
                </div>
                <div class="col-12 button-wrapper button-move-js">
                    <a class="btn-site btn-blue" href="">Acquista subito</a>
                </div>
            </div>
        </div>
    </section>
    <section class="header-section section-site-b">
        <div class="container">
            <div class="row">
                <div class="col-6 col-sm-4 col-lg-3 px-1">
                    <div class="product-wrapper">
                        <div class="image-wrapper">
                            <img src="img/home/section_01/section__el/01.png">
                        </div>
                        <div class="detail-wrapper">                            
                            <img src="img/home/section_01/section__el/logos/01.svg">
                            <div class="title-wrapper"><span>per Adulti e Bambini</span></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-sm-4 col-lg-3 px-1">
                    <div class="product-wrapper">
                        <div class="image-wrapper">
                            <img src="img/home/section_01/section__el/02.png">
                        </div>
                        <div class="detail-wrapper">
                            <img src="img/home/section_01/section__el/logos/02.svg">
                            <div class="title-wrapper"><span>per Adulti e Bambini</span></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-sm-4 col-lg-2 px-1">
                    <div class="product-wrapper">
                        <div class="image-wrapper">
                            <img src="img/home/section_01/section__el/03.png">
                        </div>
                        <div class="detail-wrapper">
                            <img src="img/home/section_01/section__el/logos/03.svg">
                            <div class="title-wrapper"><span>per Adulti e Bambini</span></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-sm-4 col-lg-2 px-1 order-lg-1">
                    <div class="product-wrapper">
                        <div class="image-wrapper">
                            <img src="img/home/section_01/section__el/04.png">
                        </div>
                        <div class="detail-wrapper">
                            <img src="img/home/section_01/section__el/logos/06.svg">
                            <div class="title-wrapper"><span>per Adulti e Bambini</span></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-sm-4 col-lg-1 px-1 order-lg-1">
                    <div class="product-wrapper">
                        <div class="image-wrapper">
                            <img src="img/home/section_01/section__el/05.png">
                        </div>
                        <div class="detail-wrapper">
                            <img src="img/home/section_01/section__el/logos/05.svg">
                            <div class="title-wrapper"><span>per Bambini</span></div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-sm-4 col-lg-1 px-1">
                    <div class="product-wrapper">
                        <div class="image-wrapper">
                            <img src="img/home/section_01/section__el/06.png">
                        </div>
                        <div class="detail-wrapper">
                            <img src="img/home/section_01/section__el/logos/04.svg">
                            <div class="title-wrapper"><span>over 60+</span></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="body-section section-site-c">
        <div class="container">
            <div class="row">
                <div class="col">
                    <div class="block-wrapper text-center d-lg-flex justify-content-lg-center align-items-lg-center">
                        <div class="title-wrapper">
                            <h2>Un prodotto Menarini, qualità italiana in farmacia</h2>
                        </div>
                        <div class="image-wrapper"><img src="img/home/section_02/Logo_MENARINI.png"></div>
                    </div>
                </div>
            </div>
        </div>

    </section>
    <section class="body-section section-site-d">
        <div class="container">
            <div class="row probiotic-row text-end">
                <div class="col">
                    <div class="text-block">
                        <div class="title-wrapper">
                            <h2><span class="small-text">Cosa sono i</span><br><span class="big-text">Probiotici?</span></h2>
                        </div>
                        <div class="text-wrapper">
                            <p>I probiotici sono microrganismi<br> preziosi che, assunti correttamente, permettono all’intestino di funzionare al meglio.</p>
                        </div>
                    </div>
                    <div class="button-wrapper"><a class="btn-site btn-blue" href="">Scopri i benefici</a></div>
                </div>
            </div>
            <div class="row kaleidon-row">
                <div class="col">
                    <div class="text-block">
                        <div class="title-wrapper">
                            <h2>Cos'è<br><span><span class="big-text">Kaleidon?</span></span></h2>
                        </div>
                        <div class="text-wrapper">
                            <p>Invece di provare 100 diverse soluzioni per ritrovare il benessere del tuo intestino, affidati al probiotico più studiato al mondo</p>
                        </div>
                    </div>
                    <div class="button-wrapper"><a class="btn-site btn-blue" href="">Scopri i benefici</a></div>
                </div>
            </div>
        </div>
    </section>
    <section class="body-section section-site-e">
        <div class="container">
            <div class="row probiotic-row text-center">
                <div class="col">
                    <div class="title-wrapper">
                        <h3>Il tuo equilibrio psico-fisico è strettamente collegato al <br><span>BENESSERE</span><br> del tuo intestino</h3>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="body-section section-site-f">
        <div class="container">
            <div class="row">
                <div class="col col-12 col-lg-4 exercise-block">
                    <div class="title-wrapper">
                        <h2>Esercizi per il<br> benessere<br> intestinale</h2>
                    </div>
                    <div class="button-wrapper"><a href="" class="btn-site btn-white">Leggi i consigli</a></div>
                </div>
                <div class="col col-12 col-lg-4 probiotic-block">
                    <div class="title-wrapper">
                        <h2>Differenze tra<br> probiotici e<br> prebiotici</h2>
                    </div>
                    <div class="button-wrapper"><a href="" class="btn-site btn-white">Leggi i consigli</a></div>
                </div>
                <div class="col col-12 col-lg-4 immune-block">
                    <div class="title-wrapper">
                        <h2>Come<br> rafforzare il<br> sistema<br> immunitario</h2>
                    </div>
                    <div class="button-wrapper"><a href="" class="btn-site btn-white">Leggi i consigli</a></div>
                </div>
            </div>
        </div>
    </section>
    <section class="body-section section-site-g">
        <div class="container">
            <div class="row">
                <div class="col text-center">
                    <div class="title-wrapper">
                        <h2>
                            Scopri dove puoi acquistare i nostri<br>
                            <span>Prodotti</span>
                        </h2>
                    </div>
                    <div class="button-block">
                        <div class="button-wrapper"><a href="" class="btn-site btn-white">Online</a></div>
                        <div class="button-wrapper"><a href="" class="btn-site btn-white">In farmacia</a></div>
                    </div>
                    <div class="image-wrapper">
                        <img src="img/home/section_04/section__el/products.png">
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="body-section section-site-h">
        <div class="container">
            <div class="row">
                <div class="col text-center">
                    <div class="title-wrapper">
                        <h2>Servizio clienti</h2>
                    </div>
                    <div class="info-block">
                        <div class="image-wrapper"><img src="img/home/section_05/infoline.svg"></div>
                        <div class="title-wrapper">
                            <h3>Il numero verde è attivo dal <br><span>Lunedì alla Domenica dalle 8:00 alle 22:00</span></h3>
                        </div>
                    </div>
                    <div class="button-block">
                        <span>Oppure</span>
                        <div class="button-wrapper"><a class="btn-site btn-blue" href="">Contattaci</a></div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</main>